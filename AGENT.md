...
- `expr @ 2025-01-01` parses to `AtDate`: the expression is evaluated with that day's rates (`getHistoricalRates` in `rates.js`). From 2024-03-02 rates come from the fawazahmed0 currency-api (all currencies, gold, crypto), falling back to Frankfurter; before that only Frankfurter/ECB (1999-01-04 on, ~30 major currencies, business days only). Missing rates are errors, never silent raw sums, and gold is never derived without an XAU price. A result that used another business day's rates carries `note`, shown as `*` with a tooltip. The first request returns `{ pending: true }` and the line shows `…`; `ratesVersion` is bumped when the download finishes, which re-runs evaluation. Days are cached in `localStorage`.
...
1. **Local Database (`src/services/localDb.js`)**:
   ...
   - **Version history** (`src/services/versionService.js`, UI in `TabHistoryModal.vue`): `snapshotTab(tab, reason, { force })` saves a tab's text. Automatic snapshots: the text before an editing burst (at most every 5 minutes), and the edited text once typing pauses for 15 s (`scheduleCheckpoint`, flushed immediately on tab switch, page hide and opening History). Forced ones are taken before clear, restore, import and cloud-sync replacement. Duplicates and empty text are skipped, 50 versions are kept per tab (15 in the localStorage fallback), and history is cleared on sign-out and local reset. It is local to the device (not synced). Call `snapshotTab(..., { force: true })` before adding any new action that overwrites tab text.
